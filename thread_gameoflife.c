...
//to teliko programma gia to game of life.o kwdikas autos ginetai compile me thn entolh gcc -o t thread_gameoflife.c -lm -pthread.
        ...
        for(f=0;f<100;f++)
	   {
		...
		now->j = l+1;
	       // printf("\t\t\t Thread Fan parameters\n\t\t\tk is %d and l is %d\n", k,l);
    	      //printf("\t\t\tto now->i exei thn timh %d\n",now->i);
	      //printf("\t\t\tto now->j exei thn timh %d\n",now->j);

	       pthread_create( &a_thread[index],NULL, (void *)&thread_fan, (void *)now);
...
void thread_fan(void * now)
{  int i,j;
   int count=0;
   struct coo *now2;//apo8hkeuoume tis times pou pernoun mesw ths klhshs ths sunarthshs.
   now2 = now;
  int h=now2->i;
  int t=now2->j;
...
//parakatw epeksergazontai ta antistoixa stoixeia me bash ths prohpo8eseis tou gameoflife kai pernoun kainourgies times analogws.Oi allages ginontai ston deutero pinaka pou exoume dhmiourghsei kai afou epistrespei h sunarthsh ,sthn main, antigrafontai ston arxiko pinaka wste na mporei na ksanaepeksergastei apo kapoio allo thread se allh fash tou problhmatos.
